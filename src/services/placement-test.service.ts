    ...
    export class PlacementTestService {
        ...
        async saveGuestResultToDatabase(testSessionId: string, studentId: number) {
            ...
            const { testId, score,answers } = tempResult;
            ...
            if (answers && answers.length > 0) {
                const answersToSave = answers.map(ans => {
                    return this.answerRepo.create({
                        answerText: ans.studentAnswer,
                        placementTestResult: result, // Tautkan ke hasil tes
                        question: { id: ans.questionId }, // Tautkan ke pertanyaan
                    });
                });
                await this.answerRepo.save(answersToSave);
                console.log(`Successfully saved ${answersToSave.length} answers for result ID ${result.id}`);
            }
